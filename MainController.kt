...
class MainController(rpc: NodeRPCConnection) {
    ...
    fun createIOU(request: HttpServletRequest): ResponseEntity<String> {
        ...
        return try {
            ...
            val signedTx = proxy.startTrackedFlowDynamic(TradeFlow.Initiator::class.java,tradeStatus,counterParty,regulator,userId,assetCode,orderType,transactionAmount,transactionFees,transactionUnits,timestamp).returnValue.getOrThrow()
            val mapper = JacksonSupport.createNonRpcMapper()
            val json = mapper.writeValueAsString(signedTx)
